...
def variant_level_assoc(pheno_type, code) -> pd.DataFrame:
    ...
    if pheno_type == "PheCode":
        phecode = code
        with open(
            UKBB_PHENO_DIR + f"phecode_cases_excludes/phecode_{phecode}.pkl", "rb"
        ) as f:
            cases = pickle.load(f)
    ...
    if not os.path.exists(f"{output_path}.fam"):
        ...
        if cases["Sex"] == "Female":
            fam.loc[
                fam["sex"] == 1, "phenotype"
            ] = -9  # if cases only applies to Female, set all Male to missing
        elif cases["Sex"] == "Male":
            fam.loc[
                fam["sex"] == 2, "phenotype"
            ] = -9  # if cases only applies to Male, set all Female to missing
        if cases["exclude"]:
    ...
    if not os.path.exists(f"{output_path}.assoc"):
        _ = subprocess.run(
            f"{plink_binary} --bed {source_plink_genome}.bed --bim {source_plink_genome}.bim --fam {output_path}.fam --assoc --out {output_path} --silent",
            shell=True,
        )
    ...
    assoc["CHR_BP_A1_A2"] = [
        f"{x}:{y}_{A1}/{A2}"
        for x, y, A1, A2 in zip(assoc.CHR, assoc.BP, assoc.A1, assoc.A2)
    ]
    assoc = assoc.merge(
    ...
    assoc = assoc[
        ["nomaly_variant", "gene_id", "RSID", "CHR_BP_A1_A2", "F_A", "F_U", "OR", "P"]
    ].sort_values("P")
